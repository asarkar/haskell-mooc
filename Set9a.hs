...
countValid = length . filter isValid
  where
    isValid xs = ((xs !! 2) == (xs !! 4)) || ((xs !! 3) == (xs !! 5))
...
sumSuccess (x : xs) = case (x, sumSuccess xs) of
  (Right x, Right y) -> Right (x + y)
  (x, y) -> if isRight x then x else y
...
data Lock = Lock {code :: String, closed :: Bool}
  deriving (Show)
...
open xs l = if xs == code l then Lock xs False else l

-- lock closes a lock. If the lock is already closed, nothing happens.
lock :: Lock -> Lock
lock l = if closed l then l else Lock (code l) True

-- changeCode changes the code of an open lock. If the lock is closed,
-- nothing happens.
changeCode :: String -> Lock -> Lock
changeCode xs l = if isOpen l then Lock xs False else l
...
compose [] _ = []
compose ((k, v) : xs) ys = case lookup v ys of
  Just x -> (k, x) : compose xs ys
  _ -> compose xs ys
