...
allValues con (Node x l r) = con x && allValues con l && allValues con r
...
cull _ Empty = Empty
cull x (Node y l r) = if x == y then Empty else Node y (cull x l) (cull x r)
...
isOrdered = go (const True)
  where
    go _ Empty = True
    go cond (Node y l r) = cond y && go (< y) l && go (> y) r
...
walk (x : xs) (Node _ l r) = walk xs $ case x of
  StepL -> l
  _ -> r
...
search val (Node x l r)
  | val == x = Just []
  | otherwise = fmap (StepL :) lPath <|> fmap (StepR :) rPath
  where
    lPath = search val l
    rPath = search val r
