...
longest :: (Ord a) => [[a]] -> [a]
-- Use the Monoid instance for Ordering to combine.
longest = minimumBy (comparing (negate . length) <> comparing head)
...
incrementKey x = map (\(a, b) -> if a == x then (a, b + 1) else (a, b))
...
transfer from to amount bank
  | Map.notMember to bank || amount <= 0 || amount > balance = bank
  -- Since - is both the infix subtract and the prefix negate,
  -- we can't use the (*x) (where * is an infix operator and x a value) syntax for -.
