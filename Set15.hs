...
statements xs ys = concatMap (\x -> liftA2 (go x) xs ys) ["is", "is not"]
  where
    go phrase x y = unwords [x, phrase, y]
...
validateAddress streetName streetNumber postCode = Address <$> name <*> number <*> code
  where
    name = check (length streetName <= 20) "Invalid street name" streetName
    number = check (isNum streetNumber) "Invalid street number" streetNumber
    code = check (isNum postCode && length postCode == 5) "Invalid postcode" postCode
    isNum = all isDigit
...
twoPersons ::
  ...
  f Bool ->
  f [Person]
twoPersons name1 age1 employed1 name2 age2 employed2 = liftA2 persons p1 p2
  where
...
normalizePhone xs = checkLen *> checkChar
  where
    ys = filter (not . isSpace) xs
    checkLen = check (length ys <= 10) "Too long" ys
    checkChar = traverse (\x -> check (isDigit x) ("Invalid character: " ++ [x]) x) ys
...
parseExpression xs = case words xs of
  [arg1, op, arg2] -> checkop op <*> checkArg arg1 <*> checkArg arg2
  _ -> invalid $ "Invalid expression: " ++ xs
  where
    checkNum x = case (readMaybe x :: Maybe Int) of
      Just i -> pure (Number i)
      _ -> invalid ("Invalid number: " ++ x)
    checkVar x =
      check
        (length x == 1 && isAlpha (head x))
        ("Invalid variable: " ++ x)
        (Variable (head x))
    checkArg x = checkNum x <|> checkVar x
    checkop x = case x of
      "+" -> pure Plus
      "-" -> pure Minus
      _ -> invalid ("Unknown operator: " ++ x)
...
instance Functor Priced where
  fmap f (Priced p v) = Priced p (f v)
