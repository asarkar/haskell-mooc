...
-- hSelectLines h nums = go 1 $ sort nums
--   where
--     go _ [] = return []
--     go lineNum ys@(x : xs) = do
--       eof <- hIsEOF h
--       if eof
--         then return []
--         else do
--           line <- hGetLine h
--           if lineNum < x
--             then go (lineNum + 1) ys
--             else do
--               lines' <- go (lineNum + 1) xs
--               return $ line : lines'

-- hSelectLines h nums =
--   hFetchLines h
--     >>= \xs -> return . map fst . filter ((`elem` nums) . snd) $ zip xs [1 ..]

hSelectLines h inds = do
  lines' <- hFetchLines h
  return $ map ((lines' !!) . pred) inds
...
interact' f state = do
  line <- getLine
  let (continue, result, state') = f (line, state)
  putStrLn result
