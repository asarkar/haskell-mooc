...
ex5_capitalize_2 = property $ do
  ...
  return . $(testing [|capitalize input|]) $ \v ->
    conjoin
      [ counterexample ("char at index " ++ show p) $ v !! p ?== toUpper (input !! p),
...
ex7_while_number = property $ do
  ...
  return $
    counterexample ("while (/=" ++ show n ++ ") (+1) 0") $
      within timeLimit $
        while (/= n) (+ 1) 0 ?== n
...
ex7_while_string = property $ do
  ...
  let w = n ++ n
      p = (/= n)
  return $
    counterexample ("while (/=" ++ show n ++ ") tail " ++ show w) $
      within timeLimit $
        while p tail w == n
...
ex8_whileRight_step = property $ do
  ...
  return $
    counterexample ("whileRight (step " ++ show limit ++ ") " ++ show start) $
      within timeLimit $
...
ex9_1 = property $ do
  ...
  where
    diffLength a b = length a /= length b
...
ex10 = $(withDefined "+|+") $ \op ->
  forAll_ $ \(x :: Int, y :: Int, xs' :: [Int], ys' :: [Int]) ->
    let xs = x : xs'
        ys = y : ys'
     in conjoin
          [ counterexample (show xs ++ " +|+ " ++ show ys) $ op xs ys ?== [x, y],
...
ex12_empty = property $ do
  ...
  let input = multiCompose [] n
      output = n
  return $
    counterexample ("multiCompose [] " ++ show n) $
      input ?== output
...
ex12_scalability = property $ do
  ...
  let input = multiCompose (replicate n (succ :: Int -> Int)) 0
      output = n
  return $
    counterexample ("multiCompose (replicate " ++ show n ++ " succ) 0") $
      input ?== output
...
ex12_arithmetic = property $ do
  ...
  let input = multiCompose [(3 *), (^ 2), (+ 1)] n
      output = ((3 *) . (^ 2) . (+ 1)) n
  return $
    counterexample ("multiCompose [(3*), (^2), (+1)] " ++ show n) $
      input ?== output
...
ex12_strings = property $ do
  ...
  return $
    counterexample ("multiCompose [(++\"xy\"), reverse, tail, reverse] " ++ show s) $
      input ?== output
...
ex13_empty = property $ do
  ...
  let input = multiApp id [] n
      output = [] :: [Int]
  return $
    counterexample ("multiApp id [] " ++ show n) $
      input ?== output
...
ex13_scalability = property $ do
  ...
  let input = multiApp (sum :: [Int] -> Int) (replicate n (succ :: Int -> Int)) k
      output = n * (k + 1)
  return $
    counterexample ("multiApp sum (replicate " ++ show n ++ " succ) " ++ show k) $
      input ?== output
...
ex13_arithmetic = property $ do
  ...
  return $
    counterexample ("multiApp sum [(+1), (*2), (`div`2), (^2)]" ++ show n) $
      input ?== output
...
ex13_strings = property $ do
  ...
  return $
    counterexample ("multiApp id [(\\xs -> [head xs]), tail, reverse]" ++ show words) $
      input ?== output

ex13_mixed =
  counterexample ("multiApp sum [head, last] [1,2,3,4]") $
...
ex14_interpreter_1 = property $ do
  ...
  return $
    counterexample ("interpreter " ++ show input) $
