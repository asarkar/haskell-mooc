      ...
      counterexample (" Initial value of r1: " ++ show w1) $
        counterexample (" Initial value of r2: " ++ show w2) $
          monadicIO $ do
            ...
            stop_ $
              conjoin
                [ counterexample " readIORef r1" $ o1 ?== w2,
        ...
        counterexample ("hSelectLines h " ++ show inds) $
          monadicIO $ do
            ...
            length (concat outs) `seq` do
              run $ hClose h
              stop_ $
                counterexample ("  was: " ++ show outs) $
                  conjoin
                    [ counterexample "Length of result" $ length outs ?== length inds,
                      conjoin
                        [ counterexample ("result!!" ++ show i) $ outs !! i ?== lines !! ((inds !! i) - 1)
                          | i <- [0 .. length inds - 1]
                        ]
                    ]
