...
average (x :| xs) = (x + sum xs) / (1.0 + fromIntegral (length xs))
...
reverseNonEmpty (x :| xs) = foldl f (x :| []) xs
  where
    f (y :| ys) z = z :| (y : ys)
...
show1 (Add1 i j) = show' i j '+'
show1 (Subtract1 i j) = show' i j '-'
show1 (Multiply1 i j) = show' i j '*'

show' :: Int -> Int -> Char -> String
show' i j sym = show i ++ [sym] ++ show j
...
instance Operation2 Add2 where
  ...
  show2 (Add2 i j) = show' i j '+'

instance Operation2 Subtract2 where
  compute2 (Subtract2 i j) = i - j
  show2 (Subtract2 i j) = show' i j '-'

instance Operation2 Multiply2 where
  compute2 (Multiply2 i j) = i * j
  show2 (Multiply2 i j) = show' i j '*'
...
passwordAllowed p (ContainsSome xs) = (not . null) $ p `intersect` xs
passwordAllowed p (DoesNotContain xs) = null $ p `intersect` xs
...
data Arithmetic = Literal Integer | Binary String Arithmetic Arithmetic
  deriving (Show)
...
evaluate (Binary sym a b) = op x y
  where
    x = evaluate a
    y = evaluate b
    op = if sym == "+" then (+) else (*)
...
render (Binary op a b) = intercalate "" ["(", x, op, y, ")"]
  where
    x = render a
    y = render b
