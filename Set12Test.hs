...
tests =
  [ (1, "incrementAll", [ex1_list, ex1_just, ex1_nothing]),
...
ex2_fmap2_examples =
  conjoin
    [ $(testing' [|fmap2 negate [[1, 2], [3]]|]) (?== [[-1, -2], [-3]]),
      $(testing' [|fmap2 head [Just "abcd", Nothing, Just "efgh"]|]) (?== [Just 'a', Nothing, Just 'e'])
    ]

ex2_fmap3_examples =
  conjoin
    [ $(testing' [|fmap3 negate [[[1, 2], [3]], [[4], [5, 6]]]|]) (?== [[[-1, -2], [-3]], [[-4], [-5, -6]]]),
...
ex4_bool =
  forAll_ $ \(bs :: [Bool]) ->
    let l = toList bs
     in counterexample ("fmap not (" ++ show l ++ ")") $
...
ex7_maybe =
  forAll_ $ \(i :: Int) ->
    forAll_ $ \j ->
      conjoin
        [ $(testing [|inBoth (Just i) (Just j)|]) (?== if i == j then [i] else []),
