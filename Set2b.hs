...
leftpad xs n = if n <= length xs then xs else ' ' : leftpad xs (n - 1)
...
countdown n = unwords ["Ready!", xs, "Liftoff!"]
  where
    xs = intercalate "... " $ map show [n, n - 1 .. 1] ++ ["..."]
...
smallestDivisor n
  ...
  | n `mod` 3 == 0 = 3
  | otherwise = case x of
      Just i -> i + (if n `mod` i == 0 then 0 else 2)
      _ -> n
  where
    -- A prime number n has no factor less than √n, and can
    -- be expressed as 6k+1 or 6k-1 for k in [1..].
    i = (floor . sqrt . fromInteger) n
    x = find (\i -> n `mod` i == 0 || n `mod` (i + 2) == 0) [5, 11 .. i]
