...
assertPhantom (DataType vars cs) =
  conjoin
    [ counterexample "  should have no constructors (be a phantom type)" $ null cs,
...
ex3_v = forAll word $ \w ->
  conjoin
    [ counterexample ("fromName (toFirst " ++ show w ++ ")") $
...
ex4_t1 =
  conjoin
    [ counterexample "capitalBob = capitalize (toFirst \"bob\")" $
...
ex4_t2 =
  conjoin
    [ counterexample "bobSmith = toFull (toFirst \"bob\") (toLast \"smith\")" $
