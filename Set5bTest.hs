...
tests =
  [ (1, "valAtRoot", [ex1_valAtRoot_Nothing, ex1_valAtRoot_Just]),
...
ex6_small =
  conjoin
    [ $(testing [|cull 2 (Node 1 (Node 2 Empty Empty) (Node 0 Empty Empty))|])
...
replaceLeaf t (Node val l r) =
  oneof
    [ fmap (\l' -> Node val l' r) (replaceLeaf t l),
...
m_ex7 depth = forAllBlind (orderedTree depth (0, 100)) $ \t ->
  forAllBlind (breakOrder t) $ \b ->
    conjoin
      [ $(testing [|isOrdered t|]) (?== True),
        isJust b ==> $(testing [|isOrdered (fromJust b)|]) (?== False)
      ]

ex7_small =
  conjoin
    [ $(testing [|isOrdered (Empty :: Tree Int)|]) (?== True),
...
ex8_small =
  conjoin
    [ $(testing [|walk [StepL] (Empty :: Tree Char)|]) (?== Nothing),
...
ex9_small =
  conjoin
    [ $(testing [|set ([] :: [Step]) 'a' (Empty :: Tree Char)|]) (?== Empty),
