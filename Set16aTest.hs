...
ex2 = forAllBlind (listOf (choose (0, 3))) $ \ns ->
  forAllBlind (vectorOf (length ns) letter) $ \cs ->
    forAllBlind (vectorOf (sum ns) letter) $ \inp ->
      let out = zip cs ns
       in conjoin
            [ $(testing [|sumIsLength inp out|]) qPass,
    ...
    forAllBlind (shuffle (zip (nub inp) ns)) $ \out ->
      forAllBlind (sublistOf inp `suchThat` (not . null)) $ \inp' ->
        forAllBlind (sublistOf out) $ \out' ->
          conjoin
            [ $(testing [|inputInOutput inp' out|]) qPass,
        ...
        forAllBlind (shuffle (outToIn out)) $ \inp ->
          forAllBlind (sublistOf out `suchThat` (not . null)) $ \out' ->
            forAllBlind (sublistOf inp) $ \inp' ->
              conjoin
                [ $(testing [|outputInInput inp out'|]) qPass,
...
ex6_1 = testGen genList $ \l ->
  conjoin
    [ counterexample "  length should be 3-5" $ length l >= 3 && length l <= 5,
...
ex6_2 = once $ testGenMany genList 30 $ \ls ->
  conjoin
    [ counterexample "  should have lists of all lengths" $ nub (sort (map length ls)) ?== [3, 4, 5],
...
ex7_arg_1 = counterexample "arbitrary :: Gen Arg" $
  testGen (arbitrary :: Gen Arg) $ \a ->
    case a of
      Number i -> counterexample "  number should be in the range 0-10" $ i >= 0 && i <= 10
      Variable c -> counterexample "  variable should be one of abcxyz" $ elem c "abcxyz"

ex7_arg_2 = once $
  counterexample "arbitrary :: Gen Arg" $
...
ex7_exp = once $
  counterexample "arbitrary :: Gen Expression" $
    testGenMany (arbitrary :: Gen Expression) 64 $ \es ->
      counterexample "  should generate all cases" $
        nub (sort (map cla es))
          ?== [ "MinusNumberNumber",
                "MinusNumberVariable",
                "MinusVariableNumber",
                "MinusVariableVariable",
                "PlusNumberNumber",
                "PlusNumberVariable",
                "PlusVariableNumber",
                "PlusVariableVariable"
              ]
  where
    cl (Variable _) = "Variable"
    cl (Number _) = "Number"
