...
outputInInput input output = forAll (elements output) count
  where
    count (x, n) = n === (length . filter (x ==)) input
...
frequenciesProp freq (NonEmpty input) = conjoin (map (($ freq input) . ($ input)) funcs)
  where
    funcs = [sumIsLength, inputInOutput, outputInInput]
...
instance Arbitrary Arg where
  arbitrary = do
    let nums = Number <$> choose (0, 10)
    let vars = Variable <$> elements "abcxyz"
    oneof [nums, vars]
