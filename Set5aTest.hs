...
ex4_name = property $ do
  n <- word
  return $
    counterexample ("getName (setName " ++ show n ++ " fred)") $
      getName (setName n fred) === n

ex4_age = property $ do
  a <- choose (0, 89)
  return $
    counterexample ("getAge (setAge " ++ show a ++ " fred)") $
      getAge (setAge a fred) === a

ex5_simple =
  conjoin
    [ $(testing' [|getX origin|]) (?== 0),
  ...
  forAllShrink_ (choose (0, 20)) $ \b' ->
    let b = a + b'
     ...
     in counterexample ("After starting from origin and going right " ++ show a ++ " times and up " ++ show b ++ " times:") $
          conjoin
            [ counterexample "getX" (getX pos1 ?== a),
...
fcmp expected actual =
  ...
  where
    diff = sum . map abs $ zipWith (-) actual expected
    eps = 0.01
...
ex8_simple = property $ do
  ...
  return $
    conjoin
      [ $(testing [|rgb (Mix (Mix c1 c2) (Mix c3 c4))|]) (fcmp [0.75, 0.25, 0]),
...
ex9 = $(reifyType "OneOrTwo") $ \(DataType vars cs) ->
  conjoin
    [ counterexample "  should have one type parameter" $ length vars ?== 1,
      counterexample "  should have 2 constructors" $ length cs ?== 2,
      counterexample "  constructor names" $
...
ex10_type = $(reifyType "KeyVals") $ \(DataType vars cs) ->
  conjoin
    [ counterexample "  type parameters" $ ["k", "v"] ?== vars,
      counterexample "  should have 2 constructors" $ length cs ?== 2,
      conjoin $ map assertNormalConstructor cs,
      counterexample "  constructor names" $
