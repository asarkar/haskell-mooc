...
instance Eq Name where
  (Name xs) == (Name ys) = map toLower xs == map toLower ys
...
instance (Eq a) => Eq (List a) where
  Empty == Empty = True
  (LNode x left) == (LNode y right) = x == y && left == right
  _ == _ = False
...
instance (Price a) => Price [a] where
  -- sum (map price xs) works too
  price = foldr (\x total -> total + price x) 0
...
instance Ord Number where
  ...
  _ <= Infinite = True -- Infinite <= Infinite => True
  _ <= _ = False
...
instance Num RationalNumber where
  (RationalNumber a b) + (RationalNumber c d) = simplify $ RationalNumber (i + j) x
    where
      x = lcm b d
      i = a * (x `div` b)
      j = c * (x `div` d)
...
class Cycle a where
  step :: a -> a
  stepMany :: Int -> a -> a
  stepMany n = foldr (.) id (replicate n step)

instance Cycle Color where
  step clr = if clr == (maxBound :: Color) then minBound :: Color else succ clr

instance Cycle Suit where
  step s = if s == (maxBound :: Suit) then minBound :: Suit else succ s
