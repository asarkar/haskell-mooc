...
tests =
  [ (1, "warmup", [ex1_nextRow, ex1_nextCol]),
    ( 2,
      "prettyPrint",
      [ ex2_examples,
        ex2_size,
    ...
    ( 3,
      ...
      [ ex3_sameRow_pos,
        ...
        ex3_sameDiag_pos,
        ex3_sameDiag_neg,
        ex3_sameAntidiag_pos,
        ex3_sameAntidiag_neg
      ]
    ),
...
ex2_examples =
  conjoin
    [ $(testing [|prettyPrint 3 [(1, 1), (2, 3), (3, 2)]|]) (?== "Q..\n..Q\n.Q.\n"),
...
ex2_size = property $ do
  ...
  return $ $(testing [|prettyPrint n xs|]) $ \out ->
    let ys = lines out
     in conjoin
          [ counterexample "  number of lines in output" $
              length ys ?== n,
...
ex3_sameAntidiag_neg = property $ do
  ...
  return $
    conjoin
      [ $(testing [|sameAntidiag (i, j) x|]) (?== False),
...
ex6_fixFirst_dangerZone = property $ do
  ...
  return $ $(testing [|fixFirst n (q : qs)|]) . was $ \x -> case x of
    ...
    Nothing ->
      counterexample
        ( "  but there were safe squares on row "
            ++ show i
            ++ ": "
  ...
  return $
    not (null s) ==> do
      ...
      return $ $(testing [|step n ((row, col) : qs)|]) . was $ \res ->
        conjoin
          [ counterexample ("  length") (length res ?== length qs + 2),
...
ex8_step_backtrack = property $ do
  ...
  return $
    not (null end) ==> do
      ...
      return $ $(testing [|step n ((row, col) : qs)|]) . was $ \res ->
        conjoin
          [ counterexample ("  length") (length res ?== length qs),
...
ex9_finish_small =
  conjoin
    [ $(testing [|finish 4 [(1, 1)]|]) (?== [(4, 3), (3, 1), (2, 4), (1, 2)]),
...
m_finish_n n = $(testing [|finish n [(1, 1)]|]) . was $ \qs ->
  conjoin
    [ counterexample "  length of list" $ length qs ?== n,
