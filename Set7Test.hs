...
tests =
  [ (1, "travel", [ex1_travel, ex1_velocity]),
...
m2_member example = forAll_ $ \(Set xs) ->
  not (null xs)
    ==> forAllBlind (elements (xs `asTypeOf` example))
    $ \x ->
      conjoin
        [ $(testing [|member x (Set xs)|]) (?== True),
          $(testing [|member x (Set (delete x xs))|]) (?== False)
        ]

ex2_member =
  conjoin
    [ m2_member [True, False],
      m2_member [1 :: Int, 2, 3]
    ]
...
ex2_empty_add =
  conjoin
    [ forAll_ $ \(i :: Integer) -> counterexample ("add " ++ show i ++ " emptySet") (add i emptySet ?== Set [i]),
      forAll_ $ \(b :: Bool) ->
        counterexample
          ("add " ++ show b ++ "(add " ++ show b ++ " emptySet)")
...
ex3_type = $(reifyType "State") $ \(DataType vars cs) ->
  conjoin
    [ counterexample "  should have no type parameters" $ vars == [],
      conjoin $ map ok cs
    ]
  where
    ok (Constructor _ []) = property True
...
ex4 =
  conjoin
    [ forAll_ $ \(Small i) -> $(testing [|average ((fromIntegral i :: Float) :| [])|]) (?== fromIntegral i),
...
ex7_mempty = $(withInstance1 "Monoid" "Set" [|((<>), mempty)|]) $ \((<>), mempty) ->
  forAll_ $ \(Set (xs :: [Char])) ->
    conjoin
      [ counterexample (show (Set xs) ++ " <> mempty") ((Set xs <> mempty) ?== Set xs),
...
ex8_show1 =
  $(hasType "show1" [t|Operation1 -> String|]) $ \show1 ->
    forAllBlind smallInt $ \i ->
      forAllBlind smallInt $ \j ->
        conjoin
          [ $(testing [|show1 (Add1 i j)|]) (?== show i ++ "+" ++ show j),
...
ex9_basics =
  conjoin
    [ $(testing [|passwordAllowed "short" (MinimumLength 8)|]) (?== False),
...
ex9_either_both = property $ do
  ...
  return $
    (low < hi) ==> do
      ...
      return $
        conjoin
          [ $(testing [|passwordAllowed (take (low - 1) password) or|]) (?== False),
...
tree leaf node cs = node (tree leaf node lefts) (tree leaf node rights)
  where
    (lefts, rights) = splitAt (length cs `div` 2) cs
    ...
    forAllShrink_ (listOf (choose ('a', 'z'))) $ \base ->
      let required = filter (not . flip elem forbidden) required'
       ...
       in not (null required)
            ==> not (null forbidden)
            ==> forAllBlind (elements required)
            $ \req ->
              forAllBlind (elements forbidden) $ \forb ->
                conjoin
                  [ $(testing [|passwordAllowed base rules|]) (?== False),
...
m_ex10 level = forAllShrink_ (choose (1, level)) $ \l ->
  forAllBlind (gen_arithmetic level) $ \(val, exp, show, code) ->
    conjoin
      [ counterexample ("evaluate " ++ code) $ evaluate exp ?== val,
