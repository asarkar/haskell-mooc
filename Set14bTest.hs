...
import Network.Wai.Handler.Warp (testWithApplication, withApplication)
              ...
              counterexample ("  deposit db (T.pack " ++ show name1 ++ ") " ++ show amount1) $
                counterexample ("  deposit db (T.pack " ++ show name2 ++ ") " ++ show amount2) $
                  monadicIO $ do
                    ...
                    stop_ $
                      counterexample "The output of: query_ db getAllQuery :: IO [(String,Int)]" $
...
ex2_big =
  ...
  where
    depo (name, amount) = "  deposit db " ++ show name ++ " " ++ show amount

ex3 =
  forAllBlind word $ \name ->
    forAllBlind (choose (0, 9999)) $ \amount ->
      conjoin
        [ counterexample ("parseCommand [T.pack \"balance\", T.pack " ++ show name ++ "]") $
...
ex8_parseCommand =
  conjoin
    [ $(testing [|parseCommand v|]) (?== Nothing)
      | v <-
          (map . map)
            T.pack
            [ [],
