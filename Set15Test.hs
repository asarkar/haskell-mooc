...
import Mooc.Th
import Set15
import Test.QuickCheck hiding (Failure, Fun, Result)
...
ex1 = forAll_ $ \i ->
  forAll_ $ \j ->
    conjoin
      [ $(testing [|sumTwoMaybes (Just i) (Just j)|]) (?== Just (i + j)),
...
ex3_success = forAllBlind (choose (-100, 100)) $ \i ->
  let s = show i
   in conjoin
        [ $(testing [|calculator "negate" s|]) (?== Just (negate i)),
...
ex4 = forAllBlind (choose (0, 100)) $ \i ->
  forAllBlind (choose (1, 10)) $ \j ->
    conjoin
      [ $(testing [|validateDiv i j|]) (?== ok (div i j)),
        ...
        forAllBlind (vectorOf 20 (choose ('a', 'z'))) $ \pad ->
          forAllBlind (shuffle (chaff : take 4 p)) $ \p' ->
            forAllBlind (shuffle (chaff : n)) $ \n' ->
              conjoin
                [ $(testing [|validateAddress (s ++ pad) n p|]) (?== invalid "Invalid street name"),
  ...
  forAllBlind word $ \name2 ->
    forAllBlind (choose (10, 100)) $ \age1 ->
      forAllBlind (choose (10, 100)) $ \age2 ->
        forAll_ $ \(emp1, emp2) ->
          conjoin
            [ $(testing [|twoPersons (Just name1) (Just age1) (Just emp1) (Just name2) (Just age2) (Just emp2)|])
      ...
      forAllBlind (choose (10, 100)) $ \age1 ->
        forAllBlind (choose (10, 100)) $ \age2 ->
          forAllBlind (choose (10, 100)) $ \age3 ->
            forAll_ $ \(emp1, emp2) ->
              conjoin
                [ $(testing [|twoPersons [name1] [age1] [emp1] [name2] [age2] [emp2]|])
...
ex7_ok = forAll_ $ \(i :: Int) ->
  forAll_ $ \(b :: Bool) ->
    conjoin
      [ $(testing [|boolOrInt (show i)|]) (?== ok (Right i)),
...
ex7_fail = forAll_ $ \(i :: Int) ->
  forAllBlind (choose ('a', 'n')) $ \c ->
    -- don't generate 0o or 0x by accident
    forAllBlind (shuffle (c : show i)) $ \ni ->
      forAll_ $ \(b :: Bool) ->
        forAllBlind (sow [c] (show b)) $ \nb ->
          conjoin
            [ $(testing [|boolOrInt ni|]) (?== invalids ["Not a Bool", "Not an Int"]),
    ...
    forAllBlind word $ \chaff ->
      forAllBlind (sow out spaces) $ \inp ->
        forAllBlind (sow inp chaff) $ \wrong ->
          conjoin
            [ $(testing [|normalizePhone inp|]) (?== ok out),
    ...
    forAllBlind (elements ["+", "-"]) $ \goodOp ->
      forAllBlind (elements ["*", "%", "x", "3"]) $ \badOp ->
        let b x op y = x ++ " " ++ op ++ " " ++ y
         ...
         in conjoin
              [ $(testing [|parseExpression (b goodArg goodOp badArg)|])
...
ex10_fmap =
  conjoin
    [ $(testing' [|fmap reverse (Priced 3 "abc")|]) (?== Priced 3 "cba"),
      $(testing' [|fmap negate (Priced 1 7)|]) (?== Priced 1 (-7))
    ]

ex10_liftA2 =
  conjoin
    [ $(testing' [|liftA2 (++) (Priced 3 "abc") (Priced 5 "de")|]) (?== Priced 8 "abcde"),
...
ex11 =
  conjoin
    [ $(testing' [|Just succ <#> Just 2|]) (?== Just 3),
...
ex12 =
  conjoin
    [ $(testing' [|myFmap negate (Just 1)|]) (?== Just (-1)),
...
ex13_maybe =
  conjoin
    [ $(testing' [|tryAll id [Nothing, Just 1, Just 2]|]) (?== Just 1),
...
ex13_list =
  conjoin
    [ $(testing' [|tryAll tail ["abc", "d", "ef"]|]) (?== "bcf"),
      $(testing' [|tryAll tail []|]) (?== ([] :: String))
    ]

tA l xs k =
  counterexample' ("tryAll (\\x -> if x>" ++ show l ++ " then pure x else invalid \"zero\") " ++ show xs) $
    k $
      tryAll (\x -> if x > l then pure x else invalid "zero") xs
...
ex13_validation = forAllBlind (choose (0 :: Int, 10)) $ \l ->
  forAllBlind (listOf1 (choose (0 :: Int, 10))) $ \xs ->
    tA
      ...
      ( ?==
          case filter (> l) xs of
            [] -> invalids (map (const "zero") xs)
            (x : _) -> ok x
      )

ex14_ok =
  conjoin
    [ $(testing' [|fmap not (Both (Just [True]))|]) (?== Both (Just [False])),
...
ex14_fail =
  conjoin
    [ $(testing' [|fmap not (Both [Nothing])|]) (?== Both [Nothing]),
      $(testing' [|fmap not (Both []) :: Both [] Maybe Bool|]) (?== Both [])
    ]

ex15_pure =
  conjoin
    [ $(testing' [|pure 1 :: Both [] Maybe Int|]) (?== Both [Just 1]),
      $(testing' [|pure "a" :: Both Maybe Validation String|]) (?== Both (Just (ok "a")))
    ]

ex15_liftA2 =
  conjoin
    [ $(testing' [|liftA2 (+) (Both (Just [10, 100])) (Both (Just [1, 2]))|]) (?== Both (Just [11, 12, 101, 102])),
      $(testing' [|liftA2 (+) (Both (Just [10, 100])) (Both Nothing)|]) (?== Both Nothing)
    ]

ex15_liftA2_2 =
  conjoin
    [ $(testing' [|liftA2 (&&) (Both (Just (invalid "err"))) (Both (Just (pure True)))|]) (?== Both (Just (invalid "err"))),
