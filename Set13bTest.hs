...
import Test.QuickCheck hiding (Failure, Result)
import Test.QuickCheck.Monadic

main = score tests

tests =
  [ (1, "ifM", [ex1_maybe, ex1_state]),
    ...
    forAll_ $ \(d :: Char) ->
      forAll_ $ \(b :: Bool) ->
        let nc = Nothing :: Maybe Char
         ...
         in conjoin
              [ $(testing [|ifM (Just b) (Just c) (Just d)|]) (?== Just (if b then c else d)),
...
ex1_state =
  conjoin
    [ $(testing' [|runState (ifM get (return 'a') (return 'b')) True|]) (?== ('a', True)),
...
ex3_visit_examples =
  conjoin
    [ $(testing' [|runState (visit maze1 "Pit") []|]) (?== ((), ["Pit"])),
...
ex3_visit = property $ do
  ...
  let maze = adj (g1 ++ g2)
  return $
    counterexample ("runState (visit " ++ show maze ++ " " ++ head names1 ++ ") []") $
      within timeLimit $
...
ex3_path_examples =
  conjoin
    [ $(testing' [|path maze1 "Entry" "Pit"|]) (?== True),
...
ex4_simple =
  conjoin
    [ $(testing [|findSum2 [6, 1] [7]|]) (hasElements [(1, 6, 7), (6, 1, 7)]),
...
ex7_monad =
  let op :: Int -> Result Int
   ...
   in conjoin
        [ counterexample' (s ++ " MkResult 1 >>= op") $
