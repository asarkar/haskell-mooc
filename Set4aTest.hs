...
m_ex2 domain = forAllBlind (sublistOf domain) $ \xs ->
  conjoin
    [ forAllBlind (shuffle xs) $ \xs' -> $(testing [|distinct xs'|]) (?== True),
...
ex8_winner = property $
  do
    names <- shuffle ["Mike", "Bob", "Lisa", "Jane"]
    ...
    return $
      conjoin
        [ $(testing [|winner inp win lose|]) (?== win),
  ...
  forAllBlind (vectorOf n arbitrary) $ \bs ->
    $(testing [|freqs bs|]) . was $ \m ->
      let (t, f) = partition id bs
          ...
          out = Map.assocs m
       in conjoin
            [ counterexample ("  Number of True values should be " ++ show ntrue) $
...
ex10_missing = property $
  do
    ...
    return $
      conjoin
        [ $(testing [|transfer present missing amount inp|]) (?== inp),
...
ex10_notEnough = property $
  do
    ...
    return $
      conjoin
        [ $(testing [|transfer from to (fromBal + amount) inp|]) (?== inp),
...
ex12_bool = property $
  do
    small <- choose (0, 100 :: Int)
    diff <- choose (1, 100)
    return $
      conjoin
        [ $(testing [|maxIndex (array (False, True) [(False, small), (True, small + diff)])|]) (?== True),
          $(testing [|maxIndex (array (False, True) [(True, small), (False, small + diff)])|]) (?== False)
        ]

ex12_int = property $
  do
    low <- choose (0, 100 :: Int)
