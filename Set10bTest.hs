...
tests =
  [ (1, "|||", [imports, ex1_normal, ex1_undefined]),
...
ex3_fail =
  conjoin
    [ counterexample "validate undefined 'c'" $ shouldFail (validate undefined 'c'),
    ...
    forAll_ $ \(i :: Int) ->
      forAllBlind (choose ('a', 'z')) $ \c ->
        $(testing [|myseq is c|]) (?== c)
          .&&. conjoin'
            [ counterexample ("myseq (undefined:" ++ show is ++ ") " ++ show c) $ counterexample "  should not fail" $ (myseq (undefined : is) c ?== c),
