...
tests =
  [ (1, "workload", [ex1_small, ex1_medium, ex1_big]),
...
ex2_examples =
  conjoin'
    [ $(testing [|echo "hello!"|]) (?== "hello!, ello!, llo!, lo!, o!, !, "),
...
ex2_random = forAllBlind word $ \w ->
  $(testing [|echo w|]) . was $ \v ->
    conjoin
      [ counterexample "  number of ',' characters" $ (length (filter (== ',') v)) ?== length w,
...
ex6_gen = property $ do
  ...
  return $
    counterexample ("with code = " ++ show code) $
      conjoin
        [ $(testing' [|isOpen (changeCode code (open "1234" aLock))|]) (?== True),
        ...
        .&&. counterexample
          ("with wrong = " ++ show wrong)
          ( conjoin
              [ $(testing' [|isOpen (open wrong (lock (changeCode code (open "1234" aLock))))|]) (?== False),
...
ex7 = $(withInstance "Eq" "Text" [|(==) :: Text -> Text -> Bool|]) $ \(==) ->
  property $ do
    ...
    return $
      conjoin
        [ $(testing [|Text bare == Text v1|]) (?== True),
