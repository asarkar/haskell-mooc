...
import Data.List
import Data.Maybe
import Language.Haskell.TH.Syntax (isInstance, lift, mkName)
import Mooc.Test
import Mooc.Th
import Set6
...
ex1 =
  conjoin
    [ testEquals Finland Finland True,
...
ex3 = property $ do
  ...
  return $
    conjoin
      [ testEquals (Name namev1) (Name namev2) True,
...
ex5_egg = $(withInstance "Price" "Egg" [|price|]) $ \price ->
  conjoin
    [ $(testing [|price ChocolateEgg|]) (?== 30),
...
ex6_egg =
  $(withInstances1 "Price" ["Maybe", "[]"] [|price|]) $ \price ->
    conjoin
      [ $(testing [|price [Just ChocolateEgg, Nothing, Just ChickenEgg]|]) (?== 50),
  ...
  $(withInstances1 "Price" ["Maybe", "[]"] [|price|]) $ \price ->
    forAllBlind (choose (0, 10)) $ \milk1 ->
      forAllBlind (choose (0, 10)) $ \milk2 ->
        conjoin
          [ $(testing [|price [Just (Milk milk1), Nothing, Just (Milk milk2)]|]) (?== (15 * milk1 + 15 * milk2)),
...
ex7_eq_check =
  $(withInstance "Eq" "Number" [|(==) :: Number -> Number -> Bool|]) $ \(==) ->
    forAll_ $ \i ->
      forAll_ $ \j ->
        conjoin
          [ $(testing [|Finite i == Finite j|]) (?== (i Prelude.== j)),
...
ex7_max_infinite =
  $(withInstance "Ord" "Number" [|max :: Number -> Number -> Number|]) $ \max ->
    forAll_ $ \i ->
      conjoin
        [ $(testing [|max (Finite i) Infinite|]) (?== Infinite),
...
ex8_eq = $(withInstance "Eq" "RationalNumber" [|(==) :: RationalNumber -> RationalNumber -> Bool|]) $ \(==) ->
  forAll_ $ \r@(RationalNumber p q) ->
    forAll_ $ \(NonZero i) ->
      forAll_ $ \(NonZero j) ->
        conjoin
          [ $(testing [|r == (RationalNumber (i * p) (i * q))|]) (?== True),
...
ex10_abs = $(withInstance "Num" "RationalNumber" [|abs :: RationalNumber -> RationalNumber|]) $ \abs ->
  forAllBlind genSimpl $ \r@(RationalNumber p q) ->
    conjoin
      [ $(testing [|abs (RationalNumber (-p) q)|]) (exactly r),
  ...
  [ (1, "Eq Country", [ex1]),
    ...
    ( 10,
      ...
      [ ex10_add_zero,
        ...
        ex10_add,
        ex10_multiply_one,
        ex10_multiply_commut,
        ex10_multiply_inverse,
        ex10_multiply,
        ex10_abs,
        ex10_signum_abs,
        ex10_fromInteger,
