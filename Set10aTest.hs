...
tests =
  [ (1, "doublify", [ex1_finite, ex1_infinite]),
...
ex3_examples =
  conjoin
    [ $(testing [|deal ["Hercule", "Ariadne"] ["Ace", "Joker", "Heart"]|]) (?== [("Ace", "Hercule"), ("Joker", "Ariadne"), ("Heart", "Hercule")]),
...
ex4_simple =
  conjoin
    [ $(testing [|averages [1.0, 2.0, 3.0]|]) (approximateListEq [1.0, (1 + 2) / 2, (1 + 2 + 3) / 3]),
...
ex5_examples =
  conjoin
    [ $(testing' [|take 20 (alternate "abc" "def" ',')|]) (?== "abc,def,abc,def,abc,"),
...
shuffleCase w =
  ( do
      s <- vectorOf (length w) (elements [toLower, toUpper])
      return (zipWith ($) s w)
  )
    `suchThat` (/= w)
  ...
  $( do
       ...
       case n of
         ...
         Just n -> do
           info <- reify n
           case info of
             TyConI (NewtypeD _ _ _ _ _ _) -> [|property True|]
...
ex9_1 = forAllShrink_ (choose (0, 20)) $ \i ->
  counterexample ("with i = " ++ show i) $
    conjoin
      [ $(testing' [|play maze (replicate i "Left")|]) (?== take (i + 1) (cycle ["Maze", "Deeper in the maze", "Elsewhere in the maze"])),
