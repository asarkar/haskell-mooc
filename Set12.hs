...
count x = foldr (\y s -> s + fromEnum (y == x)) 0
...
inBoth xs = foldr (\y acc -> if F.elem y xs then y : acc else acc) []
...
instance Foldable List where
  ...
  foldr f initial (LNode x xs) = f x $ foldr f initial xs
...
instance Foldable TwoList where
  foldr _ initial TwoEmpty = initial
  foldr f initial (TwoNode x y xs) = f x $ f y (foldr f initial xs)
